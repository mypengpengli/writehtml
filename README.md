...
- **认知与保密**：设定可分别记录客观真相、读者已知内容、角色认知或误解、保密要求，避免 AI 提前泄底。
- **证据**：AI 派生的变化默认保存对应正文片段，可从画布跳回来源位置；可在画布设置中关闭证据展示。

### AI 分析与确认

AI 写入正文、执行章节复核，或作者主动点「分析本章」时，系统会用当前模型整理场景和状态变化。场景与普通的状态进展自动保存；新设定、重大变化和可能改写正史的内容只进入“待确认”，作者接受后才成为正式设定。手工修改旧章节时，下游章节会被标记为待复核，不会静默改写后续正文。

这项分析会额外消耗一次当前文字模型请求。画布设置中的「离开未分析章节时提醒」默认开启，它只提醒，不会在每次键入时调用模型；关闭后仍可手动分析。

### Agent 工具

AI 助手可直接使用 `list_story_cards`、`save_story_card`、`save_story_card_state`、`list_chapter_scenes`、`save_chapter_scene` 和 `analyze_chapter_production`。画布和正文共用当前 AI 会话与模型，选择卡片或场景后点「交给 AI」只把该对象作为下一回合目标，不会把整个画布重复塞进上下文。已确认且与当前章节有效的设定摘要会进入写作上下文。
