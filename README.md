...
- **创作生产画布**：独立工作区正式拆成「设定库 / 剧情规划 / 章节状态」。设定库管理人物、规则、地点、技能、道具和组织；剧情规划管理总纲、卷纲、阶段、章节、场景、支线与伏笔；章节状态展示“章前状态 → 正文场景 → 变化 → 章后状态”。AI 助手仍共用右侧同一会话，可把任意卡片、计划或场景设为本轮目标。
...
- **作品设定（bible）**：旧版自由文本仍完整保留；升级后可明确选择“保留为设定 / 整体导入剧情规划 / 仅作参考”。系统不会按标题猜测并拆分旧内容。
...
画布服务于长篇小说的连续性和故事规划，不替代正文编辑器，也不是视频生产流水线。顶层只有三个职责明确的入口：设定库保存世界事实，剧情规划保存作者已经选择的未来，章节状态保存正文派生结果。顶栏的「编辑正文」可随时回到当前章节，右侧仍使用同一个 AI 助手。
...
- `planned`：已经进入正式计划；`committed`：作者锁定方向；`realized`：已由正文实现；`abandoned`：保留历史但不再使用。
...
剧情推演仍使用独立沙盘。标准流程是 `候选 candidate → 采纳为 planned → 锁定 committed → 写正文 → World State 提取事实 → 核对 realized/deviated`。采纳不是把未来直接写成正史，也不再把计划摘要伪装成本章实际剧情摘要。
