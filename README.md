...
- **可视化大纲 / 情节沙盘**：一个作品可建多棵互不影响的分支树；可同步真实章节和历史分支、拖动与整理节点、手工加分支，或让 AI 给出“发散 / 收束 / 推进”三种候选。AI 展开会读取当前分支路径与已确认 World State；沙盘试错不改正文，主动采纳后才创建正式章节并带入节点摘要、分支计划和章节目标。
- **创作生产画布**：在独立工作区查看全书章节时间线，以及本章的“规则 → 章前状态 → 场景 → 变化 → 章后状态”。人物、规则、地点、技能、道具和组织都保留稳定基础设定与逐章状态版本；AI 助手仍常驻右侧，可把任意卡片或场景设为本轮操作目标。
...
画布服务于长篇小说的连续性管理，不替代正文编辑器，也不是视频生产流水线。桌面端采用可拖动画布与两侧抽屉，手机端自动改为纵向流程；顶栏的「编辑正文」可随时回到当前章节。
...
画布卡片、场景表单和节点布局采用“本机草稿 → 延迟同步数据库”两级保存。断电、刷新或网络失败时，未同步内容仍留在当前浏览器；再次打开同一对象会恢复草稿。已有对象关闭详情时会先尝试完成最后一次同步，新对象仍由作者点击创建后进入数据库。

### Agent 工具

AI 助手可直接使用 `list_story_cards`、`save_story_card`、`save_story_card_state`、`list_chapter_scenes`、`save_chapter_scene`、`analyze_world_state` 和兼容别名 `analyze_chapter_production`。画布和正文共用当前 AI 会话与模型，选择卡片或场景后点「交给 AI」只把该对象作为下一回合目标，不会把整个画布重复塞进上下文。已确认且与当前章节有效的设定摘要会进入写作上下文。
...
3. 「＋作品」「＋章」建好目标；可顺手在「⋯」👥实体 建人物/地点卡片、给人物生成形象并在「角色图库」管理历史图，在作品设定写大纲。常用文风或审稿要求可在「⋯」→「AI Skills」存成模板。
